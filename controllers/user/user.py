...
def read_users(service: UserService = Depends(), current_user=Depends(get_auth_user)):
    return service.read()


@user_router.get("/{user_id}", status_code=200, response_model=UserResponse)
def get_user_by_id(
    user_id: UUID, service: UserService = Depends(), current_user=Depends(get_auth_user)
):
...
def get_user_by_email(
    email: str, service: UserService = Depends(), current_user=Depends(get_auth_user)
):
